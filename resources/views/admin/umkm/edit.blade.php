...
<div class="flex items-center gap-4 mb-6">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500">{{ $umkm->nama }}</p>
    </div>
</div>
    ...
    <form action="{{ route('admin.umkm.update', $umkm->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama" value="{{ old('nama', $umkm->nama) }}" placeholder="Nama usaha / toko" class="input-field @error('nama') input-error @enderror">
            ...
            <div>
                ...
                <select name="kategori" class="input-field @error('kategori') input-error @enderror">
                    ...
                    <option value="{{ $kat }}" @selected(old('kategori', $umkm->kategori)==$kat)>{{ $kat }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nama_pemilik" value="{{ old('nama_pemilik', $umkm->nama_pemilik) }}" placeholder="Nama pemilik usaha" class="input-field @error('nama_pemilik') input-error @enderror">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat usaha..." class="input-field @error('deskripsi') input-error @enderror">{{ old('deskripsi', $umkm->deskripsi) }}</textarea>
                @error('deskripsi')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Alamat <span class="text-red-500">*</span></label>
                <input type="text" name="alamat" value="{{ old('alamat', $umkm->alamat) }}" placeholder="Alamat usaha" class="input-field @error('alamat') input-error @enderror">
                @error('alamat')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">RT <span class="text-red-500">*</span></label>
                <input type="text" name="rt" list="rt-list" value="{{ old('rt', $umkm->rt) }}" placeholder="Contoh: 001" maxlength="3" class="input-field @error('rt') input-error @enderror">
                <datalist id="rt-list">
            ...
            <div>
                ...
                <input type="text" name="no_hp" value="{{ old('no_hp', $umkm->no_hp) }}" placeholder="08xxxxxxxxxx" class="input-field @error('no_hp') input-error @enderror">
                @error('no_hp')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Jam Operasional</label>
                <input type="text" name="jam_operasional" value="{{ old('jam_operasional', $umkm->jam_operasional) }}" placeholder="Contoh: 08.00 - 17.00" class="input-field @error('jam_operasional') input-error @enderror">
            ...
            <div class="md:col-span-2">
                ...
                <input type="url" name="maps_link" value="{{ old('maps_link', $umkm->maps_link) }}" placeholder="https://maps.app.goo.gl/..." class="input-field @error('maps_link') input-error @enderror">
            ...
            <div class="md:col-span-2">
                ...
                @if($umkm->foto)
                <div class="mb-2">
                    <img src="{{ asset('storage/'.$umkm->foto) }}" alt="{{ $umkm->nama }}" class="w-24 h-24 rounded-lg object-cover">
            ...
            <div class="md:col-span-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $umkm->is_active) ? 'checked' : '' }} class="w-4 h-4 text-rose-600 rounded border-slate-300">
