...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 max-w-xl">
    <form action="{{ route('admin.kegiatan.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="deskripsi" rows="4" class="input-field" placeholder="Jelaskan kegiatan yang akan dilaksanakan">{{ old('deskripsi') }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal') }}" class="input-field @error('tanggal') input-error @enderror">
                    @error('tanggal')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Jam Mulai</label>
                    <input type="time" name="waktu" value="{{ old('waktu') }}" class="input-field">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="tempat" value="{{ old('tempat') }}" placeholder="Contoh: Aula RW, Balai Warga" class="input-field">
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="status" class="input-field @error('status') input-error @enderror">
                <option value="akan_datang" @selected(old('status')=='akan_datang')>Akan Datang</option>
