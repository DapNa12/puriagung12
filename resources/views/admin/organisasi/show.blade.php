...
<div class="flex items-center gap-4 mb-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Detail Anggota Organisasi</h1>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 max-w-lg">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            ...
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold mt-1">{{ $pengurus->organisasi }}</span>
        </div>
    </div>
    ...
    <dl class="grid grid-cols-2 gap-4 text-sm">
        <div>
            ...
            <dd class="text-slate-900 font-semibold mt-1">{{ $pengurus->jabatan }}</dd>
        </div>
        <div>
            <dt class="text-slate-500 font-medium">Organisasi</dt>
            <dd class="text-slate-900 font-semibold mt-1">{{ $pengurus->organisasi }}</dd>
        </div>
        <div>
    ...
    <div class="flex space-x-3 mt-6 pt-4 border-t border-slate-100">
        <a href="{{ route('admin.organisasi.edit', $pengurus->id) }}" class="btn-soft-yellow">
            <i data-lucide="pencil" class="w-4 h-4"></i> Edit
        </a>
        <form action="{{ route('admin.organisasi.destroy', $pengurus->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus anggota ini?')">
            @csrf @method('DELETE')
