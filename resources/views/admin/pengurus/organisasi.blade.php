                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-4 py-3.5 text-sm text-right">
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <form action="{{ route('admin.pengurus.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus anggota ini?')">
                                @csrf @method('DELETE')
...
<div class="sm:hidden space-y-3">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-4">
        ...
        <div class="flex items-center gap-1.5 pt-2 border-t border-slate-100 mt-2">
            <a href="{{ route('admin.pengurus.show', $item->id) }}" class="btn-soft-blue">Detail</a>
            <a href="{{ route('admin.pengurus.edit', $item->id) }}" class="btn-soft-yellow">Edit</a>
            <form action="{{ route('admin.pengurus.destroy', $item->id) }}" method="POST" class="inline ml-auto" onsubmit="return confirm('Yakin ingin menghapus anggota ini?')">
                @csrf @method('DELETE')
                <button type="submit" class="btn-soft-red">Hapus</button>
            </form>
        </div>
