...
<head>
    ...
    <title>@yield('title', 'Dashboard') - Admin Puri Agung Permai RW12</title>
...
<body class="bg-slate-50 h-screen flex overflow-hidden">
    <aside id="sidebar" class="fixed md:static inset-y-0 left-0 z-50 bg-white border-r border-slate-200 flex flex-col transform -translate-x-full md:translate-x-0 md:transition-none transition-transform duration-300 ease-in-out h-full md:h-auto flex-shrink-0">
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
            ...
            <a href="{{ route('admin.dashboard') }}" class="sidebar-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}" data-title="Beranda">
            ...
            <a href="{{ route('admin.statistik.index') }}" class="sidebar-link {{ request()->routeIs('admin.statistik*') ? 'active' : '' }}" data-title="Statistik Kependudukan">
            ...
            <a href="{{ route('admin.users.index') }}" class="sidebar-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}" data-title="Kelola User">
                <i data-lucide="users" class="w-5 h-5 flex-shrink-0"></i>
            ...
            <a href="{{ route('admin.berita.index') }}" class="sidebar-link {{ request()->routeIs('admin.berita*') ? 'active' : '' }}" data-title="Berita">
                <i data-lucide="megaphone" class="w-5 h-5 flex-shrink-0"></i>
                <span>Berita</span>
            </a>
            <a href="{{ route('admin.galeri.index') }}" class="sidebar-link {{ request()->routeIs('admin.galeri*') ? 'active' : '' }}" data-title="Galeri">
                <i data-lucide="camera" class="w-5 h-5 flex-shrink-0"></i>
                <span>Galeri</span>
            </a>
            <a href="{{ route('admin.activity-log.index') }}" class="sidebar-link {{ request()->routeIs('admin.activity-log*') ? 'active' : '' }}" data-title="Log Aktivitas">
        ...
        <div class="user-section border-t border-slate-100 p-3 flex-shrink-0">
            <div class="flex items-center gap-3 px-3 py-2.5 user-info">
                <div class="w-8 h-8 bg-rose-100 rounded-full flex items-center justify-center flex-shrink-0">
                    <span class="text-sm font-semibold text-rose-700">{{ substr(Auth::user()?->name ?? '', 0, 1) }}</span>
                </div>
            ...
            <div class="user-actions mt-1 flex border-t border-slate-100 pt-2">
                <a href="{{ route('home') }}" class="flex items-center justify-center gap-1.5 px-3 py-2 text-xs text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all w-full" data-title="Ke Website">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm h-14 flex items-center px-4 md:px-6 sticky top-0 z-40">
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-base md:text-lg font-bold text-slate-900">@yield('title', 'Dashboard')</h1>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative flex items-center gap-2.5 pl-3 border-l border-slate-200">
                        ...
                        <button id="user-dropdown-btn" type="button" class="w-9 h-9 bg-gradient-to-br from-rose-500 to-rose-700 rounded-full flex items-center justify-center shadow-sm flex-shrink-0 hover:from-rose-600 hover:to-rose-800 transition-all cursor-pointer focus:outline-none" title="Menu pengguna">
                            <span class="text-xs md:text-sm font-semibold text-white">{{ substr(Auth::user()?->name ?? '', 0, 1) }}</span>
                        </button>
