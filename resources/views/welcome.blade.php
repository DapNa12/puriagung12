    ...
    <div class="relative max-w-7xl mx-auto px-4 py-20 md:py-28">
        <div class="text-center max-w-3xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('pengumuman') }}" class="inline-flex items-center justify-center gap-2 bg-white text-rose-800 px-8 py-3 rounded-xl font-semibold hover:bg-rose-50 transition-all shadow-lg shadow-rose-900/20">
                    <i data-lucide="megaphone" class="w-5 h-5"></i>
                    Lihat Pengumuman
                </a>
                <a href="{{ route('kegiatan') }}" class="inline-flex items-center justify-center gap-2 bg-rose-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-rose-500 transition-all border border-rose-400/30 shadow-lg shadow-rose-900/20">
...
<div class="max-w-7xl mx-auto px-4 -mt-10 relative z-10">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-6 text-center">
            ...
            <div class="text-3xl font-bold text-gray-900">{{ $totalWarga }}</div>
        ...
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-6 text-center">
            ...
            <div class="text-3xl font-bold text-gray-900">{{ $pengumuman->count() }}</div>
        ...
        <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-6 text-center">
            ...
            <div class="text-3xl font-bold text-gray-900">{{ $kegiatan->count() }}</div>
...
<div class="max-w-7xl mx-auto px-4 py-16">
    ...
    <div class="mb-16">
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('pengumuman') }}" class="text-rose-600 hover:text-rose-700 font-medium text-sm hidden sm:block">Lihat semua &rarr;</a>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($pengumuman as $item)
            <a href="{{ route('pengumuman.show', $item) }}" class="group bg-white rounded-2xl shadow-sm hover:shadow-md border border-gray-100 p-6 transition-all duration-200">
                <div class="flex items-center gap-2 text-xs text-gray-400 mb-3">
                    <span>{{ $item->created_at->isoFormat('D MMMM Y') }}</span>
                    <span>&middot;</span>
                    <span>{{ $item->user->name ?? 'Admin' }}</span>
                </div>
                <h3 class="font-bold text-gray-900 group-hover:text-rose-700 transition-colors mb-2">{{ $item->judul }}</h3>
                <p class="text-gray-600 text-sm leading-relaxed">{{ Str::limit($item->isi, 120) }}</p>
        ...
        <a href="{{ route('pengumuman') }}" class="text-rose-600 hover:text-rose-700 font-medium text-sm mt-4 inline-block sm:hidden">Lihat semua pengumuman &rarr;</a>
    </div>
    @endif
    ...
    <div>
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('kegiatan') }}" class="text-rose-600 hover:text-rose-700 font-medium text-sm hidden sm:block">Lihat semua &rarr;</a>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-start gap-4">
                    <div class="w-14 h-14 bg-rose-50 rounded-xl flex flex-col items-center justify-center flex-shrink-0">
                        <span class="text-lg font-bold text-rose-700 leading-none">{{ \Carbon\Carbon::parse($item->tanggal)->format('d') }}</span>
                        <span class="text-xs text-rose-600 font-medium">{{ \Carbon\Carbon::parse($item->tanggal)->locale('id')->isoFormat('MMM') }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="font-bold text-gray-900 mb-1">{{ $item->nama_kegiatan }}</h3>
                        <div class="flex flex-wrap gap-3 text-sm text-gray-500">
                            @if($item->waktu)<span class="flex items-center gap-1"><i data-lucide="clock" class="w-4 h-4"></i>{{ $item->waktu }}</span>@endif
                            @if($item->tempat)<span class="flex items-center gap-1"><i data-lucide="map-pin" class="w-4 h-4"></i>{{ $item->tempat }}</span>@endif
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-16 text-center bg-gradient-to-r from-rose-50 to-rose-100 rounded-2xl p-8 md:p-12">
        ...
        <a href="{{ route('pengurus-rt') }}" class="inline-flex items-center gap-2 bg-rose-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-rose-500 transition-all shadow-lg shadow-rose-200">
